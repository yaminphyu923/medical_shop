                                        ...
                                        <div class="card-body order-card">
                                            ...
                                            <div class="d-flex justify-content-end">
                                                <a href="{{ route('add-session-cart', $medical_list->id) }}">
                                                    <button type="button"
                                                        class="btn btn-order btn-info">Order</button>
                                                </a>
                                            </div>
                    ...
                    <form action="{{ route('orders.store') }}" method="POST">
                        ...
                        <div class="d-flex justify-content-between my-3">
                            ...
                            <div class="justify-content-end">
                                <button type="submit" class="btn btn-success btn-sm cal"
                                    @if (session()->get('cart') == '') disabled @endif>Calculate</button>
                                {{-- <button type="submit" class="btn btn-success btn-sm"
                                    @if (!session()->get('cart')) disabled @endif>Save&Print</button> --}}
                            </div>
                        </div>
                        ...
                        <table class="table table-stripped table-hover">
                            ...
                            <tbody>
                                @if (session()->get('cart') != '')
                                    @forelse(session()->get('cart') as $key=>$medical_list)
                                        <tr>
                                            <td>{{ $medical_list['name'] }}</td>

                                            <td>
                                                <input type="hidden" name="medical_id[]"
                                                    value="{{ $medical_list['id'] }}">
                                                <select name="price[]" id="" class="form-control">
                                                    @forelse($medical_list['prices'] as $medical_price)
                                                        <option value="{{ $medical_price->price }}">
                                                            {{ $medical_price->price }}</option>
                                                    @endforeach
                                                </select>
                                            </td>

                                            <td>{{ date('d-M-Y', strtotime($medical_list['expired_date'])) }}</td>
                                            <td>
                                                <div class="row">
                                                    <div class="col-sm-6">
                                                        <input type="text" id="quantity_value"
                                                            value="{{ $medical_list['qty'] }}" name="qty[]"
                                                            class="form-control" />

                                                    </div>

                                                    {{-- @php $subtotal = $medical_list['qty'] * $medical_price->price  @endphp

                                                    @php
                                                        $grandtotal = 0;
                                                        $grandtotal += $medical_list['qty'] * $medical_price->price;
                                                    @endphp --}}

                                                    <div class="col-sm-1">

                                                        <a
                                                            href="{{ route('remove-session-cart', $medical_list['id']) }}">
                                                            <button type="button"
                                                                class="trash btn btn-danger btn-sm"><i
                                                                    class="fa fa-trash"
